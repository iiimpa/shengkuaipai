<template>
  <div class="content flexs j_center">
    <div class="box">
      ...
      <div class="login_r">
        ...
        <div class="login_inp">
          ...
          <input placeholder="请输入手机号" maxlength="11" v-model="form.cell" type="text"/>
        </div>
        <div @click="regist" class="btn cur_p">注册</div>
        <div style="cursor: pointer;font-size: 14px; color: #008bfe;" @click="showProtocol=true">《用户协议》</div>
        <div class="db">
          已有账号？
    ...
    <el-dialog title="用户注册使用协议" :visible.sync="showProtocol">
      <h4>前言</h4>
      本网站由深圳市以太亮点区块链科技有限公司所有和负责运营。

      <h4>一、用户注册</h4>
      <p>1、为了使用升排名提供的服务，用户需要创建一个升排名账户。</p>
      <p>
        2、注册用户具备完全民事权利能力和与所从事的民事行为相适应的行为能力的自然人、法人，或者是具有合法经营资格的实体组织。无民事行为能力人、限制民事行为能力人以及无经营或特定经营资格的组织不当注册为升快排用户或超过其民事权利或行为能力范围而从事交易的，其与升快排之间的服务协议自动无效，一经发现，升快排有权立即注销该用户，并追究其使用升快排中一切“服务”的任何法律责任。如您代表一家公司或其他法律主体在升快排注册登记，则您声明和保证，您有权使该公司或该法律主体受本协议“条款”的约束。</p>
...
<script>
  export default {
    ...
    methods: {
      ...
      regist() {
        if (!/^1[3456789]\d{9}$/.test(this.form.cell)) {
          this.$message({
            message: '手机号错误，请重新输入！',
            type: 'warning'
          });
        ...
        this.form.invite = this.$route.query.invite ? parseInt(this.$route.query.invite) : 0;
