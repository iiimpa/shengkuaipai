...
def pc360(keyword, domain, proxy, ua, resolution, delay):
    ...
    try:
        ...
        # 定位到text的文本框
        text = myweb.find_element_by_id("input")

        # 搜索词条
        text.send_keys(keyword)

        # 定位到搜索键
        button = myweb.find_element_by_id("search-button")
        # 点击搜索键
        button.click()
        ...
        while page < 10 and not finded:
            ...
            arr = myweb.find_elements_by_tag_name("cite")
            for item in arr:
            ...
            if not finded:
                ...
                next = myweb.find_element_by_id("snext")
