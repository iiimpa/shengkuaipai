import os,time,json,datetime,random,queue,requests
...
class Constructor(object):
	...
	def __init__(self, data,token):
		...
		self.fixed_click  = data['fixed_click']
	...
	#构建任务
	def createTask(self):
		if(self.fixed_click != 0):
			self.exeCondi('%Y-%m-%d 00:00:00','%Y-%m-%d 23:59:59',self.fixed_click)
		elif(self.clickLimtCount['min'] != '0' or self.clickLimtCount['max'] != '0'):
	...
	def createParams(self,time_begin,time_over):
		...
		cost = 1
		self.tasks.append({'mode':self.mode,'optimization':self.optimization,'token':self.token,'order_id':int(self.id),'status':0,'schedule_time':str(self.randomtimes(time_begin,time_over)[0]),'cost':cost,'domain':self.domain,'platform':int(self.platform),'jump_times':jump_times,'stay_time':stay_time,'keyword':self.keyword,'xiongzhang':self.xiongzhang})
